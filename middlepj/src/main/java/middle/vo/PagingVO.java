...
public class PagingVO {
	...
	private int startPage; // 게시글 화면(페이지네이션)에 보여질 첫번째 번호
	private int endPage; // 게시글 화면(페이지네이션)에 보여질 마지막 번호
	...
	public PagingVO(int pageNum, int amount, int total) {
...
//		 공식 = (int)Math.ceil(전체게시글수 / 화면에보여질데이터개수)
		int realEnd = (int)Math.ceil(this.total / (double)this.amount); // 상품게시판 끝번호
